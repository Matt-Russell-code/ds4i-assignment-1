...
data <- data %>%
  mutate(OAH = ifelse(OAH == "", NA, as.character(OAH)),
         OAH = factor(OAH))   # rebuild factor from scratch, no "" level
...
::: {=html}
<!--#include file="cnn_filter_animation_quarto_ready_html"-->
:::
...
# 
# 
# saved_path <- h2o.saveModel(best_model, path = getwd(), force = TRUE)
...
best_model #H20 FFN final  model chosen
#cnn_final_model #cnn model 
#rnn_final_model #final  rnn model

best_pred_df  #h20 ffn model predictions on test data

pred_ffn_labels <- best_pred_df[,"predict"]
# pred_cnn
# pred_rnn

pred_ffn <- factor(pred_ffn, levels = levels(true_labels))
...
#so the  final prediction objects are
...
true_labels # for  FFn
temporal_labels  # for cnn  and rnn
