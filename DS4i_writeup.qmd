...
## Introducion and Literature Review
...
While the present study used only complete cases for modelling, avalanche forecasters in practice must often make predictions despite missing data. Imputation was considered, but fell outside the scope of evaluating NN performance. Developing an imputation model for this dataset could be a way of meaningfully improving its practical use, robustness, and accuracy overall.

There are several other improvements that can be made going forward:

1.  Address class imbalance through data generation or sampling techniques to better capture rarer high-risk hazards.
2.  Incorporate climate change effects by accounting for evolving weather and snowpack conditions alongside historical data.
3.  Explore building region-specific models to reflect Scotland's highly localised weather patterns and improve forecast reliability.
4.  Explore alternative model types, such as gradient boosting or hybrid deep learning ensembles, to better handle categorical data and strengthen predictions.
