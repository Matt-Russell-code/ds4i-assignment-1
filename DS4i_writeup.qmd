...
We want to apply pca to our wine data for...

Customer Segment is our dependant variable, in the original data this column is not a customer segment but the place of origin of the wine. But let's imagine we are working for a wine business owner.

The customer segment are customers and the specific wine that they like, we have 3 segments of customers. So our goal now is to take all the wine details and the corresponding customer segment that likes it and then build a classification model where we can predict based on wine feature details which customer segment to recommend this wine to.
```{r libs-and-seed}
...
train_h2o_split <- splits[[1]]  # used for model fitting
valid_h2o_split <- splits[[2]]  # used for early stopping / model selection
```
...
# Create a fresh train/validation split inside H2O for grid search / early stopping
...
write.csv(data.frame(auc=auc_list), "h2o_nn_per_class_auc.csv")
```


```{r}
h2o.varimp(best_h2o_model)

h2o.varimp_plot(best_h2o_model)
```
