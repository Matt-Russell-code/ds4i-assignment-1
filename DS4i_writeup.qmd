...
```{r }
# 
# #set window = 3 days, h = 1
# #Windows to test, 3, 7, 30, 120
# w_ <- pre_data(w = 3, h = 1)
# gc()
# y.test <- w_[[1]]
# y.train <- w_[[2]]
...
# plot(history)
# 
# pred_probs <- model |> predict(X.test)
# pred_class <- apply(pred_probs, 1, which.max)   # 1–6
# true_class <- apply(y.test, 1, which.max)
# 
# head(true_class)
# library(caret)
# confusionMatrix(as.factor(pred_class), as.factor(true_class))
# 
# 
# load("test1")
# 
...
# 
# 
# input <- layer_input(shape = c(3, F_))
# 
# 
# output <- input |>
...
# pred_probs <- model |> predict(X.test)
# pred_class <- apply(pred_probs, 1, which.max)   # 1–6
# true_class <- apply(y.test, 1, which.max)  # 1..K
# 
# 
# 
# confusionMatrix(as.factor(pred_class), as.factor(true_class))
