...
kable(as.data.frame(cnn_cm$byClass), caption = "CNN Statistics by Class", digits = 4) %>% kable_styling()
```

```{r tbl-rnn_stats_by_class, echo = FALSE}

kable(as.data.frame(rnn_cm$byClass), caption = "RNN Statistics by Class", digits = 4) %>% kable_styling()
```

```{r tbl-ffn_stats_by_class, echo = FALSE}

kable(as.data.frame(ffn_cm$byClass), caption = "FFNN Statistics by Class", digits = 4) %>% kable_styling()
```
...
#| fig-cap: "Variable Importance for FFN-MLP."
