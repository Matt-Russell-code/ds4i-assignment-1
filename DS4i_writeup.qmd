...
author: Don Gwese, Nina Lewis, Sanana Mwanawina, Matthew Russell, and Aidan Thomas
format:
  html:
    css: style.css
execute:
...
library(patchwork)
library(gridExtra)
# library(keras3)  #Does  not work on all collarborators  machines. 
# library(tensorflow)
# library(reticulate)
...
[Photo by Nicolas Cool (2016) on Unsplash]{style="font-size:12px"}
...
1.  Few published studies exist (only 9 relevant peer-reviewed papers exist on Scopus, with the most recent study from 2011 (keywords: ( "avalanche forecasting" OR "avalanche prediction" ) AND Scotland)).
...
::: {style="font-size: 0.6em; font-style: italic"}
...
Lastly, @fig-FAH-OAH shows the relationship between *forecast.avalanche.hazard (FAH)* (day *t*) and *OAH* (day *t+1*). Each column sums to 100%, showing how outcomes were distributed for each forecast level. "Low" forecasts were highly accurate (`r round(tab_prop[1,1]*100, 2)`%), while intermediate levels ("Moderate", "Considerable-") had weaker agreement, often spilling into neighbouring categories. "High" forecasts were least reliable, frequently over-predicting extreme conditions.
...
@tbl-missing-val presents the percentage of missing values for features with incomplete data. *Av.Cat* was removed because its meaning was unclear and it contained implausible values (e.g., `r x1`). @fig-ski-summit-wind shows no clear relationship between *ski.penetration* and *OAH*, except for "High" risk avalanches; we therefore excluded it since snowfall effects are likely better represented by other variables. *Summit.wind.direction* (`r round(missing_df[missing_df$Variable == "Summit.Wind.Dir", "Missing (%)"], 2)`% missing) was also dropped for lack of a discernible trend.
...
px <- ggplot(data, aes(x = OAH, y = Ski.Pen)) +
  ...
  ylim(0, 20) + theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
After removing variables, `r comp`% of samples remained incomplete and were subsequently dropped. While retaining more observations would be ideal, the remaining \~7000 were sufficient for representative results.
...
::: {style="font-size: 0.6em; color: black; text-align: left; font-style: italic"}
...
#function to do the grid searches for me
...
grid <- data.frame(
  hyperparameter = c("filters (layer 1)", "filters (layer 2)", "kernel size", "dropout (layer 1)", "dropout (layer 2)"),
  values = I(list(
    c(16, 32, 64),
    c(16, 32, 64),
    c(5, 15),
...
grid_summary <- data.frame(
  Hyperparameter = c("units (layer 1)", "units (layer 2)", "dropout (layer 1)", "drop (layer 2)"),
  Values = c(
...
From the expanded grid-search, the best performing model achieved a validation accuracy of `r round(max(res$val_accuracy) * 100, 1)`%. We fitted this model to the full training data and used that to predict onto the test set. Overall, we were satisfied with this search as the validation accuracy had not improved drastically compared to the initial window search; for example, the best model in the first stage reached roughly `r x1`%. The final RNN model therefore included 64 LSTM units in the first layer, 16 units in the second layer with 70% dropout in the first and 50% dropout in the second. 

```{r final_rnn_search}


#Final search for hyperparameters for RNN
...
The randomised grid-search evaluated a maximum of `r n1` candidate models, exploring a predefined hyperparameter space. Key hyperparameters and their search ranges are detailed in @tbl-ffnn-param .
...
acc_FOH <- round(mean(
  as.character(test$OAH) == as.character(test$FAH),
  na.rm = TRUE
) *100)
```
...
@fig-compare_cms displays the confusion matrices for the three models. FFNN-MLP has a clearer diagonal, indicating stronger performance. The confusion matrix for RNN is very diffused with widespread misclassifications. @tbl-overall_stats confirms this: RNN had the lowest accuracy (`r acc_rnn`%) and FFNN-MLP had the highest (`r acc_ffn`%). FFNN-MLP accuracy just narrowly beats the accuracy of the forecasted values (*FAH)* (`r acc_FOH`%). Due to the training and testing split used for the CNN and RNN (where data was neither shuffled nor randomly assigned), "High" risks, being rare, did not end up appearing in the test set. Consequently, the models' ability to identify "High" risk cases could not be evaluated.
...
# Function to plot confusion matrices using CM objects
...
# p3 <- plot_cm(ffn_cm, "FFNN-MLP Confusion Matrix")
# 
...
# FFN confusion matrix reordered
ffn_cm_df <- data.frame(
  Prediction = c("Low", "Moderate", "Considerable -", "Considerable +", "High"),
  Low                = c(536, 112, 4, 1, 0),
  Moderate           = c(17, 369, 103, 1, 1),
  Considerable_neg = c(2, 41, 250, 17, 10),
  Considerable_pos  = c(0, 6, 56, 14, 3),
  High               = c(0, 0, 10, 1, 14)
)

colnames(ffn_cm_df) <- c("Prediction", "Low", "Moderate", "Considerable -", "Considerable +", "High")

# CNN confusion matrix reordered
cnn_cm_df <- data.frame(
  Prediction = c("Low", "Moderate", "Considerable -", "Considerable +", "High"),
  Low                = c(735, 205, 1, 8, 0),
  Moderate           = c(148, 270, 3, 44, 0),
  Considerable_neg = c(18, 60, 4, 32, 0),
  Considerable_pos  = c(8, 18, 3, 9, 0),
  High               = c(0, 0, 0, 0, 0)
)

colnames(cnn_cm_df) <- c("Prediction", "Low", "Moderate", "Considerable -", "Considerable +", "High")

# RNN confusion matrix reordered
rnn_cm_df <- data.frame(
  Prediction = c("Low", "Moderate", "Considerable -", "Considerable +", "High"),
  Low                = c(752, 175, 0, 22, 0),
  Moderate           = c(219, 165, 2, 78, 0),
  Considerable_neg = c(53, 29, 0, 31, 0),
  Considerable_pos = c(20, 5, 0, 12, 0),
  High               = c(0, 0, 0, 0, 0)
)

colnames(rnn_cm_df) <- c("Prediction", "Low", "Moderate", "Considerable -", "Considerable +", "High")

plot_cm <- function(cm_df, title) {
  # Convert wide-format df to long format for ggplot
  cm_long <- melt(cm_df, id.vars = "Prediction", variable.name = "Reference", value.name = "Freq")
  
  # Ensure the factor levels keep the desired order
  cm_long$Prediction <- factor(cm_long$Prediction, levels = cm_df$Prediction)
  cm_long$Reference  <- factor(cm_long$Reference, levels = colnames(cm_df)[-1])
...
# Plot them
p1 <- plot_cm(cnn_cm_df, "CNN Confusion Matrix")
p2 <- plot_cm(rnn_cm_df, "RNN Confusion Matrix")
p3 <- plot_cm(ffn_cm_df, "FFNN-MLP Confusion Matrix")
...
get_overall_stats <- function(cm_obj) {
  data.frame(
    ...
    Recall  = round(mean(cm_obj$byClass[,"Sensitivity"], na.rm = TRUE), 4),
    Specificity  = round(mean(cm_obj$byClass[,"Specificity"], na.rm = TRUE), 4),
    Precision    = round(mean(cm_obj$byClass[,"Pos Pred Value"], na.rm = TRUE), 4)
    )
}

# Build comparison table
model_comparison <- rbind(
  FFNN_NLP = get_overall_stats(ffn_cm),
...
Looking at performance statistics by class can help us gain useful insight into each model's strengths and weaknesses, providing a more detailed overview of model behaviour. @tbl-cnn_stats_by_class, @tbl-rnn_stats_by_class, and @tbl-ffn_stats_by_class summarise our model statistics by class.
...
cnn_cm_df <- as.data.frame(cnn_cm$byClass)[class_order, ]

kable(cnn_cm_df, format  = "html", 
      caption = "CNN Statistics by Class", 
      digits = 4) %>%
  kable_styling("hover", "striped") %>%
  footnote(general = "All values rounded to 4 decimal places.")
```

```{r tbl-rnn_stats_by_class, echo = FALSE}

rnn_cm_df <- as.data.frame(rnn_cm$byClass)[class_order, ]

kable(rnn_cm_df, caption = "RNN Statistics by Class",format  = "html", 
      digits = 4) %>%
  kable_styling("hover", "striped") %>%
  footnote(general = "All values rounded to 4 decimal places.")
```
...
#| fig-cap: "Variable importance for FFNN-MLP."
...
6.  Cool, N. 2016. *Caution avalanche danger signage during winter*. Unsplash. Available: <https://unsplash.com/photos/caution-avalanche-danger-signage-during-winter-Cd2QnIKU6dk> \[2025, September 24\].

7.  Diggins, M. 2009. The challenges for Scottish avalanche forecasters observing a maritime snowpack. *Proceedings of the International Snow Science Workshop*. 22-24. Available: <https://arc.lib.montana.edu/snow-science/objects/issw-2009-0024-0026.pdf> \[2025, September 10\].

8.  Fromm, R. & Schonberger, C. 2022. Estimating the danger of snow avalanches with machine learning approach using a comprehensive snow cover model. *Machine Learning with Applications*. 10. DOI: 10.1016/j.mlwa.2022.100405

9.  Gauthier, F., Laliberte, J. & Meloche, F. 2025. Assessing the predictive capability of several machine learning algorithms to forecast snow avalanches using numerical weather prediction model in eastern Canada. *EGUsphere*. DOI: 10.5194/egusphere-2025-1572

10. Heierli, J., Purves, R.S., Felber, A. & Kowalski, J. 2004. Verification of nearest-neighbours interpretations in avalanche forecasting. *Annals of Glaciology*. 38:84-88. DOI: 10.3189/172756404781815095

11. Helbig, N., van Herwijnen, A. & Jonas, T. 2015. Forecasting wet-snow avalanche probability in mountainous terrain. *Cold Regions Science and Technology*. 120:219-226. DOI: 10.1016/j.coldregions.2015.07.001

12. Hendrick, M., Techel, F., Volpi, M., Olvski, T., Perez-Guillen, van Herwijnen, A. & Schweizer, J. 2023. Automated prediction of wet-snow avalanche activity in the Swiss Alps. *Journal of Glaciology*. 69(277):1365-1378. DOI: 10.1017/jog.2023.24

13. Kala, M., Jain, S., Singh, A. & Krishnan, N.C. 2025. Addressing class imbalance in avalanche forecasting. *Cold Regions Science and Technology*. 231. DOI: 10.1016/j.coldregions.2024.104411

14. Langmuir, E. 1970. Snow profiles in Scotland. *Weather*. 25(5):205-209. DOI: 10.1002/j.1477-8696.1970.tb03262.x

15. MetOffice. 2010. *UK regional climates*. Available: <https://www.metoffice.gov.uk/research/climate/maps-and-data/regional-climates/index> \[2025, September 11\].

16. OpenAI. (2025a). ChatGPT (Sept 23 version) \[Large language model\]. https://chatgpt.com/share/68d59a8c-efa4-8004-a295-616e83f37afb

17. OpenAI. (2025b). ChatGPT (Sept 23 version) \[Large language model\]. https://chatgpt.com/share/68d59ad3-d50c-8004-b862-18966277b186

18. Pozdnoukhov, A., Purves, R.S. & Kanevski, M. 2008. Applying machine learning methods to avalanche forecasting. *Annals of Glaciology*. 49:107-113. DOI: 10.3189/172756408787814870

19. Pozdnoukhov, A., Matasci, G., Kanevski, M. & Purves, R.S. 2011. Spatio-temporal avalanche forecasting with Support Vector Machines. *Nat. Hazards Earth Syst. Sci.* 11:367-382. DOI: 10.5194/nhess-11-367-2011

20. Purves, R.S., Morrison, K.W., Moss, G. & Wright, D.S.B. Nearest neighbours for avalanche forecasting in Scotland - Development, verification and optimisation of a model. *Cold Regions Science and Technology*. 37(3):343-355. DOI: 10.1016/S0165-232X(03)00075-2

21. Rahmati, O., Ghorbanzadeh, O., Teimurian, T., Mohammadi, F., Tiefenbacher, J.P., Falah, F., Pirasteh, S., Ngo, P.T.T. & Bui, D.T. 2019. Spatial modeling of snow avalanche using machine learning models and geo-environmental factors: Comparison of effectiveness in two mountain regions. *Remote Sensing*. 11(24). DOI: 10.3390/rs11242995

22. Schirmer, M., Lehning, M. & Schweizer, J. 2009. Statistical forecasting of regional avalanche danger using simulated snow-cover data. *Journal of Glaciology*. 55(193):761-768. DOI: 10.3189/002214309790152429

23. Scottish Avalanche Information Service. 2024. *Reviews of the winter season*. Available: <https://www.sais.gov.uk/sais-annual-reports/> \[2025, September 13\].

24. Scottish Avalanche Information Service. 2025. *Avalanche information for the Scottish mountains*. Available: <https://www.sais.gov.uk/> \[2025, September 5\].

25. Scottish Government. 2011. *Scotland's marine atlas: Information for the National Marine Plan*. Available: <https://www.gov.scot/publications/scotlands-marine-atlas-information-national-marine-plan/pages/7/> \[2025, September, 10\].

26. Sharma, V., Kumar, S. & Sushil, R. 2023. A neural network model for automated prediction of avalanche danger level. *Nat. Hazards Earth Syst. Sci.* 23:2523-2530. DOI: 10.5194/nhess-23-2523-2023

27. Singh, A. & Ganju, A. 2008. Artificial neural networks for snow avalanche forecasting in Indian Himalaya. *Proceedings of the 12th International Conference of International Association for Computer Methods and Advances in Geomechanics (IACMAG)*. 1-6 October 2008. Gao, India. 1664-1670. Available: <https://www.researchgate.net/publication/263651736_Artificial_Neural_Networks_for_Snow_Avalanche_Forecasting_in_Indian_Himalaya> \[2025, September 13\].

28. Spink, P.C. 1970. Scottish snowbeds in summer 1969. *Weather*. 25(5): 201-204. DOI: 10.1002/j.1477-8696.1970.tb03261.x

29. Tiwari, A., Arun, G. & Vishwakarma, B.D. 2021. Parameter importance assessment improves efficacy of machine learning methods for predicting snow avalanche sites in Leh-Manali Highway, India. *Science of The Total Environment*. 794. DOI: 10.1016/j.scitotenv.2021.148738

30. Tu, J.V. 1996. Advantages and disadvantages of using artificial neural network versus logistic regression for predicting medical outcomes. *Journal of Clinical Epidemiology*. 49(11):1225-1231. DOI: 10.1016/S0895-4356(96)00002-9

31. Ward, R.G.W. 1980. Avalanche hazard in the Cairngorm Mountains, Scotland. *Journal of Glaciology*. 26(94):31-41. DOI: 10.3189/S0022143000201020

32. Ward, R.G.W. 1984a. Avalanche prediction in Scotland: I. A survey of avalanche activity. *Applied Geography*. 4(2):91-108. DOI: 10.1016/0143-6228(84)90016-X

33. Ward, R.G.W. 1984b. Avalanche prediction in Scotland: II. Development of a predictive model. *Applied Geography*. 4(2):109-133. DOI: 10.1016/0143-6228(84)90017-1

34. Ward, R.G.W., Langmuir, E.D.G. & Beattie, B. 1985. Snow profiles and avalanche activity in the Cairngorm Mountains, Scotland. *Journal of Glaciology*. 31(107):18-27. DOI: 10.3189/S0022143000004949

35. Webster, H. 2020. *Avalanches in Scotland - interview with Mark Diggins*. Walkhighlands. Available: <https://www.walkhighlands.co.uk/news/avalanches-in-scotland-interview-with-mark-diggins/> \[2025, September 10\].

36. Wen, H., Wu, X., Liao, X., Wang, D., Huang, K. & Wunnemann, B. 2022. Application of machine learning methods for snow avalanche susceptibility mapping in the Parlung Tsangpo catchement, southeastern Qinghai-Tibet Plateau. *Cold Regions Science and Technology*. 198. DOI: 10.1016/j.coldregions.2022.103535

37. Werritty, A. & Sugden, D. 2012. Climate change and Scotland: Recent trends and impacts. *Earth and Environmental Science Transactions of The Royal Society of Edinburgh*. 103(2):133-147. DOI: 10.1017/S1755691013000030

38. WSL Institute for Snow and Avalanche Research SLF. 2016. *Scotland - Avalanche warning experts exchange*. Available: <https://www.slf.ch/en/news/scotland-avalanche-warning-experts-exchange/?utm_source=chatgpt.com> \[2025, September 12\].
