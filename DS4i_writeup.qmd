...
risk_levels <- c("low", "moderate", "considerable -", "considerable +", "high")
...
# Make sure true labels and FFN predictions are factors with the right levels
true_labels <- factor(tolower(as.character(true_labels)), levels = risk_levels)
pred_ffn_labels <- factor(tolower(as.character(pred_ffn_labels)), levels = risk_levels)

# Map CNN and RNN integer predictions (1–5) back to factor labels
pred_cnn_labels <- factor(risk_levels[pred_cnn], levels = risk_levels)
pred_rnn_labels <- factor(risk_levels[pred_rnn], levels = risk_levels)

# Also ensure temporal labels are factors with same levels
temporal_labels_cnn <- factor(risk_levels[temporal_labels_cnn], levels = risk_levels)
temporal_labels_rnn <- factor(risk_levels[temporal_labels_rnn], levels = risk_levels)
