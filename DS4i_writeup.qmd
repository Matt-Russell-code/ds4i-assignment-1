...
The diagonal of the RNN is the weakest of them all.Its correct predictions are concentrated in Low (792) with spillover to Moderate (62), and its misclassifications are concentrated in Low (232) and High (165) predictions which were actually Moderate instances. This tells us the model often confused Moderate cases as either Low (underestimated hazard) or High (overestimated hazard).

In the case of the CNN, almost all predictions go to Low. There is a noticeable concentration on the diagonal for Low classifications (786), and a considerable concentration of Moderate classifications (256). On the off-diagonal, the model misclassified 150 Low instances as Moderate, and 173 Moderate instances as Low. Clearly, the CNN collapsed into predicting these two classes in many cases, and often confused the two classes. Also, it completely fails to predict High or Considerable+ classes. The rows for these respective classes are all zeros. This shows that this model fell for imbalance bias, favouring the majority class Low and second most abundance Moderate.
...
The FFNN's specificity was 0.934, the highest value observed. This tells us that this model is very goof at ruling out cases that do not belong to a class. Its recall was 0.618, which means the model correctly identifies ~62% of true cases or in other words, it catches more actual danger levels compared to CNN and RNN. The CNN and RNN specificity scores were 0.881 and 0.847 respectively, which indicates they have fair ability to avoid false alarms of avalanche hazards, but not up to par with the FFNN's ability. Notable, the RNN's precision was 0.282, which means when it predicts, its only correct abour 28% of the time. This means the RNN is wrong more often than its not. Contrast that with the CNN's 0.550 precision (correct a little over half the time) and the FFNN's 0.633 (correct around 63% of the time).
...
The CNN statistics detailed in @tbl-cnn_stats_by_class show that this model had mized results. It was able to capture 83% of Low danger cases, which is the majority class, but its performnace dropped sharply for the minority cases. It has 0% sensitivity for High and Considerable+ cases meaning it completely failed to identify these two. Additionally, it only detected 22% of Considerable- cases.

Even on a class-by-class basis, the RNN performed poorly overall. It was able to catch about 83% of Low hazard cases. Beyond that, its performance collapsed. It missed all Considerable-, Considerable+, and High cases (zero sensitivity across these classes). It only managed to detect 13% of Moderate cases. Moreover, its specificity for the Low class was just 53%, meaning it frequently mislabeled other classes as Low. Just poor performance overall.
...
The FFNN turned out to be the best-performing model overall. It showed the most balanced detection and was the only one that managed to give acceptable performance across hazard levels. The CNN struggled with the minority classes and mainly acted as a majority-class (Low hazard) detector. This is unsatisfactory because in a real forecasting setting, it's of great importance that we can detect the high-risk cases. The RNN underperformed across the board, missing nearly all the minority class instances (High, Considerable+, Considerable-), and doing _fairly_ well with the majority cases but still not as well as the other two models. This strongly disagrees with our initial hypothesis that sequence models would benefit from time series dependencies.

### Strenghts and Weaknesses

A clear strenght across all models is that they handled the majority class fairly well. This suggests that with a more balanced dataset where all classes have enough representation, similar architectures could perform much better. On the other hand, all three models showed clear bias toward the most common categories. None of them did especially well with the high-risk, minority classes. The highlights a weakness in applying these models directly without addressing the class imbalance.

### Improvements & Future Contributions

There are several improvements that can be made going forward:

- Directly addressing the class imbalance by further exploring data generation or sampling techniques to help the models learn more about the high-risk hazard levels that are rare in the current dataset.
- Literature shows that climate change is altering avalanche formation and risk. This means that future models need to take into acccount not just the historical data, but the shifting weather and snowpack patterns.
- Scotland has very localised weather conditions where patterns in the eastern region can differ greatly from the west. Building seperate models for different regions or microclimates may improve local accuracy and forecast reliability.
- Exploring alternative model types could strengthen predictions. Ensemble techniques such as Gradient Boosting or hybrid deep learning ensembles could potentially handle the categorical nature of this data better than the implemented architectures.
- Further experimentation showed that training a model on a dataset completed with simple imputation performed about the same in terms of accuracy as the model trained only on complete cases. This finding is important because, in real-world forecasting, decisions often need to be made with incomplete information. Exploring more imputation techniques could therefore be a valuable direction for improving model robustness
